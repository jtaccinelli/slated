---
name: refine-role
description: Refine an existing role through targeted interview — adjusting specific sections without rebuilding from scratch. Nothing is written until every proposed change is confirmed.
...
Make targeted, minimal changes to an existing role definition. The casting director reads the current role in full, interviews to understand exactly what needs to change and why, proposes specific edits to the affected sections only, and writes the updated file after explicit confirmation. No section is touched that is not in scope.
...
### Step 4 — Write the updated role file

Apply only the confirmed changes to the role file at the location it was found. Every section not in scope must remain exactly as it was.

---

## Output

- The role file at its original location — updated with targeted changes only
